...
function setTweens()
{
	...
	if ( !effectController.useQuaternion ) {
		...
		// use quaternions on the object
			hat.quaternion.copy( startQuaternion );
...
function setupGui() {
	...
	effectController = {
		...
		shadowDarkness: ( light.shadow && light.shadow.darkness !== undefined ? light.shadow.darkness : 0.5 ),
	...
	gui.add( effectController, "shadowDarkness", 0.0, 1.0, 0.1 ).name( "shadow darkness" ).onChange( function() {
		if ( light.shadow ) {
			light.shadow.darkness = effectController.shadowDarkness;
		}
	});
