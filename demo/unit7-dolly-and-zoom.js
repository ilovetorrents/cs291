...
function render() {
	...
	eyedir.subVectors( camera.position, cameraControls.target );
	camera.fov = (180/Math.PI)*2*Math.atan(eyeTargetScale/eyedir.length());
	camera.near = eyedir.length()/100;
	camera.far = eyedir.length() + 10000;
