...
function setupGui() {
	...
	gui.add( effectController, "animation", ["step","timed","bulletproof","wave"] ).name("animation");
}
...
function render() {
	...
	if ( newTime > oldTime + 1/effectController.fps )
	{
		...
		if ( effectController.animation === "wave" ) {
			angle = (speed * clock.getElapsedTime()) % 250;
			if ( angle < 125 ) {
				// go from -22 to 103 degrees
				bird.animated.rotation.z = (angle - 22) * Math.PI/180;
				// set only so that turning off bulletproof gives no jump
				tiltDirection = 1;
			} else {
				// go back from 103 to -22 degrees
				// Take angle, 125-250, map to PI to 0, convert to radians, take cosine
				var cosVal = Math.cos( ((250-angle) / 125) * Math.PI );
				var power = 1;
				cosVal = ( cosVal >= 0 ) ? Math.pow(cosVal,power) : -Math.pow(-cosVal,power);
				// stretch back value -1 to 1 to range 125-250.
				angle = (125/2) - (125/2) * cosVal;
				bird.animated.rotation.z = (angle - 22) * Math.PI/180;
				// set only so that turning off bulletproof gives no jump
				tiltDirection = -1;
			}
		} else if ( effectController.animation === "bulletproof" ) {
			angle = (speed * clock.getElapsedTime()) % 250;
			if ( angle < 125 ) {
				// go from -22 to 103 degrees
				bird.animated.rotation.z = (angle - 22) * Math.PI/180;
			...
			if ( effectController.animation === "timed" ) {
				bird.animated.rotation.z += tiltDirection * speed * renderDelta * Math.PI/180;
			} else {
				// effectController.animation === "step"
				bird.animated.rotation.z += tiltDirection * 0.5 * (speed/30) * Math.PI/180;
			}
